...
## [2026-09-03] lint | AI DE 강의 원본 PDF 40개를 파트·순서대로 재배치

`raw/data-engineering/` 루트에 평평하게 깔려 있던 강의 PDF 40개를
**`raw/data-engineering/ai-de-course/part1..part5/`** 로 옮기고, 파일마다 **강의 순서대로 2자리
번호**를 붙였다. 파일명 규칙이 파트마다 달라(`CH02-7.` · `4.` · `Part2_Ch 3` · `01. LLM…`)
`ls` 정렬이 수강 순서와 어긋나던 것이 원인 — 이제 정렬 순서 = 수강 순서다.

- **번호 부여 근거는 새로 만들지 않았다.** [[AI Data Engineering (Fast Campus course)]] 트래커가
  이미 갖고 있던 파트·챕터 매핑을 그대로 썼다. 원본 파일명 규칙 표는 *복원 근거*로서 트래커에
  보존하고, `현재 위치` 열을 덧붙였다.
- **파일명 처리:** Part 1은 원본 파일명 유지(중복되는 원본 번호만 제거) · Part 2~4는 챕터 제목이
  파일명에 없었으므로 덱에서 가져와 붙임(`03. Ch3. ML 데이터·서빙 파이프라인.pdf`) · Part 5는
  원본 제목 유지.
- **위키 인용 경로 일괄 갱신:** source 페이지 63개의 `sources:` 프론트매터와 본문 인용 128군데를
  새 경로로 치환. 축약 표기 6건(`CH02-1. …까지 1.pdf`, `1.~3. …1~3.pdf` 등 실제 파일명이 아닌
  생략형)은 손으로 고쳐 `part1/` 디렉토리 + 번호 범위로 바꿨다.
- **검증:** 위키가 인용하는 raw PDF 경로 41개 전부 실존(broken 0), 디스크의 덱 40개 전부 최소
  한 페이지에서 인용됨(uncited 0).
- **부수 발견 — 덱 수 오기 정정:** 트래커 머리말이 "41개 슬라이드 덱"이었으나 파트별 합은
  25+5+5+2+3 = **40**이다. 페이지 합계(205+206+273+431+40 = 1,155p)도 40 기준으로 맞는다.
  같은 폴더에 있던 Apache 책 PDF를 함께 센 것으로 보인다 → **40**으로 정정.

`raw/` 는 gitignore 대상이라 이 재배치 자체는 버전 관리되지 않는다 — 커밋에 남는 것은 트래커의
배치표와 소스 페이지들의 새 인용 경로다. **이 엔트리보다 앞선 log 엔트리들은 재배치 이전 경로를
인용한다** (append-only 원칙에 따라 수정하지 않음).

## [2026-09-03] query | 이 강의 목차로 온전히 학습하면 나은 DE가 되나

**질문:** [[AI Data Engineering (Fast Campus course)]]의 목차를 커리큘럼으로 볼 때, 이걸 온전하게
학습하면 더 나은 데이터 엔지니어가 되는가.

**답:** 된다 — 단 **"판단할 수 있는 DE"** 가 되지 **"만들 수 있는 DE"** 가 되지는 않는다.
학습자의 문제가 아니라 목차가 그렇게 생겼다. → [[AI DE course as a DE curriculum]]

목차만으로 나오는 판정 3개: **실습 0**(코드 실린 덱 0/40, 그런데 "실습"이라 이름 붙은 유일한 챕터
Part 3 Ch5조차 26p 전부 개념) · **도구 챕터가 8p**(전체의 0.7%, `Great Expectations` 0회) ·
**AI 특화 55% vs DE 코어 45%**(제목 그대로 AI DE지 DE가 아니다).

목차에 통째로 없는 축 5종(오케스트레이션 운영 · SQL 실행계획 · SCD · 데이터 품질 도구 ·
IaC/CI/비용/보안)은 **[[Wiki gap analysis - DE readiness]] §2.3이 위키 자체 진단으로 먼저 도달한
목록과 같다.** 두 경로가 같은 답에 닿은 것이 핵심 — **강의를 더 파도 안 닫힌다.**

반대편도 기록했다: 파트 간 논지가 한 축으로 이어지는 것(어휘 → 시스템 설계 → 의미 → 물리 제약 →
모델)은 도구 나열형 커리큘럼에 없는 것이고, **이 코스의 실제 산출물은 지식이 아니라 판단 축**이다.

"온전하게" 학습하는 것의 함정도 표로 남겼다 — **코스 내부 모순 4건**(Redis CAP 분류 · 4h vs 8h ·
RAG "99% 정확도" vs Part 3의 mismatch · BoW 대체 vs BM25 현역)과 사실 오류 3건.
→ **파트별로 신뢰도를 다르게 잡는다.**

배경(커머스 백엔드 + 인프라 운영) 기준 배분표를 붙였다: **⭐⭐ 이상만 349p로 전체의 30%**
(Part 2 전체 · Part 4 Ch4 GPU · Part 4 Ch1-3 CAP · Part 5 덱 3). 나머지는 복습이거나 보류거나
위키 페이지가 원문보다 나은 구간.
