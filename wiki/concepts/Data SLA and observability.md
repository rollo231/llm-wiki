...
### 품질을 재는 도구 — Griffin과 그 자리

**Apache Griffin**은 배치·스트림 데이터에 품질 규칙을 정의하고 측정·모니터링하는 프레임워크다.
null 비율·중복 건수·값 범위 같은 규칙을 정해 두고 **파이프라인이 실행될 때마다 점수나 위반 내역을
기록한다**(Spark 등과 함께 돌리는 방식이 흔하다). 대안: Great Expectations · dbt tests · 클라우드 DQ.

⭐ **이 도구가 앉는 자리가 [[Data catalog and semantic layer]] §거버넌스 삼각형의 "믿을 만한가" 축이다** —
*"Atlas가 '무엇인지', Ranger가 '누가 보는지'라면 Griffin은 '믿을 만한지'를 확인한다."*

⚠️ **"Griffin만 도입한다고 품질 문화가 생기지는 않는다."** 먼저 정할 것 둘:
**어떤 테이블을 점검 대상으로 둘지**, 그리고 **기준을 못 지키면 멈출지 알릴지.**
시작은 *null 비율·중복 건수처럼 단순한 규칙 몇 개* 로 충분하다.
...
⭐⭐ **그 전에 정할 것이 하나 있다** — *"기준을 못 지키면 **적재를 멈출지 알리기만 할지**를 먼저 정해
두면 된다."* 서킷 브레이커는 기술이 아니라 이 결정이고, 결정하지 않으면 알림만 쌓인다
(위 §경고 피로) → [[Apache Map - Ch10 Governance and BI]]
