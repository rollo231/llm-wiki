...
거버넌스라고 하면 정책 문서만 떠올리기 쉽지만, 데이터 플랫폼에서는 **세 가지 질문이 반복된다.**

| 질문 | 축 | Apache 조합 | 현대 대안 |
|---|---|---|---|
| **무엇이 있는가** | 카탈로그·분류·계보로 **"알기"** | 🔹 **Atlas** | DataHub · OpenMetadata · [[Apache Polaris]] · Gravitino |
| **누가 볼 수 있는가** | 권한·정책·감사로 **"막기·남기기"** | 🔸 **Ranger** | 클라우드 IAM · 웨어하우스 자체 권한 · Lake Formation |
| **믿을 만한가** | 품질 지표로 **"재기"** | ▪️ **Griffin** | Great Expectations · dbt tests · 클라우드 DQ |

> **"제품 이름이 바뀌어도 이 세 질문은 거의 그대로 남는다."**

⚠️ **"한 도구에 세 축을 모두 넣으면 처음에는 편해 보여도 역할이 모호해지기 쉽다."**
(위 표에서 Unity Catalog가 세 칸에 다 등장하는 것이 같은 문제의 다른 얼굴이다.)

⭐⭐ **처방은 설치 순서가 아니라 자기 진단이다** — *"제품 설치 순서보다 **우리 팀에서 비어 있는 축이
어디인지** 확인하는 편이 좋다. 카탈로그만 있고 권한 통제가 약하거나, 권한만 엄격하고 품질 조건이
없으면 데이터 신뢰성이 떨어진다."*
...
- **중앙 정책** — Hive·HDFS·Kafka처럼 연동된 서비스에 정책을 배포하고 허용·거절을 **같은 기준**으로 적용.
- **세밀 통제** — 테이블뿐 아니라 **컬럼·행 단위**. Atlas 태그와 결합해 *"민감 태그가 붙은 컬럼은 특정
  역할만 조회"* 같은 규칙을 만든다 → **분류(Atlas)와 통제(Ranger)가 맞물리는 지점.**
- **감사** — 누가 무엇을 접근했는지 추적.

⚠️ **연동되지 않은 엔진에는 정책이 적용되지 않는다.** 클라우드 IAM·웨어하우스 자체 권한과 경계가
겹칠 수도 있다. ⭐ 그래서 정해야 할 것은 하나다 — **"최종 권한 기준이 어디에 있는가."**

## ⭐ 카탈로그는 단일 제품이 아니라 역할 구조다

위 3분류(metastore / data catalog / semantic layer)에 이어, **metastore 칸 안에서도 역할이 갈린다.**

| | 범위 |
|---|---|
| **Atlas** | 설명·분류·계보 중심의 **전통 카탈로그** — 사람용에 가깝다 |
| **[[Apache Polaris]]** | **Iceberg 특화** REST 카탈로그. *"예전 Hive Metastore가 맡던 역할을 Iceberg REST 카탈로그 방식으로 이어가는 흐름"* |
| **Gravitino** | 파일·테이블·**모델** 등 여러 자산, 여러 엔진·여러 카탈로그가 섞인 환경의 **통합** |

⭐⭐ 그래서 물어야 할 것은 제품 이름이 아니다 — **"우리 테이블의 공식 정보가 어디에 등록되는가."**
⚠️ 이미 다른 카탈로그를 쓰고 있다면 **같은 테이블을 두 곳에 등록해 정보가 어긋나지 않도록** 역할을 나눈다.
