...
Part 2의 [[AI DE 강의 2-01 데이터 파이프라인의 진화와 데이터 엔지니어]]가 같은 서사를 한 번 더 한다 — 저장·연산이
비쌌으니 넣기 전에 "작고 예쁘게 깎았고", 클라우드 DW의 연산력과 스토리지 비용 급감이 "일단 다 쌓아 두자"를 가능하게
했다. 새로 더하는 것은 ETL의 단점 한 줄("변환 로직이 바뀌면 파이프라인 전체를 수정")과 도식의 **EtLT**인데, EtLT는
그림에만 있고 설명은 없다.
