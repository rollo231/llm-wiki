...
- **컨텍스트 엔지니어링은 LLM 시스템의 피처 엔지니어링이고, 그래서 스큐 문제가 되돌아온다.** 인덱싱 때 쓴 청킹 방식·
  임베딩 모델과 질의 때 쓰는 임베딩 모델이 다르면, 에러 없이 검색 품질만 떨어진다 — [[학습-서빙 스큐]]와 같은 모양의
  조용한 실패다. retrieval 설정(top-k·rerank·청킹)은 "피처 로직"이므로 [[AI DE 강의 2-06 Training-Serving Skew 예방]]이
  스케일링 파라미터를 모델과 함께 배포할 아티팩트로 고정하라고 한 것처럼 버전으로 묶어야 한다. 강의는 변경 지점 목록을
  주지만 둘을 잇지 않는다. *(위키의 연결)* → [[데이터와 모델 버전 관리]]
- **"막을 곳은 모델이 아니라 retrieval 계층"은 데이터 계층의 책임이다.** 권한 기반 retrieval은 RAG에서의 접근 통제 —
  Part 1 거버넌스의 권한 관리(RBAC·마스킹)가 벡터 DB 조회 필터로 옮겨 온 것이다. 이 강의의 보안 대응 네 가지 중 셋
  (권한 기반 retrieval, 컨텍스트 정화, 툴 입력 스키마 검증)이 모델 밖의 데이터 경로에서 일어난다. *(위키의 연결)*
  → [[데이터 거버넌스와 카탈로그]]의 "권한 관리가 검색 계층으로 옮겨 간다"
- **변경 지점 목록이 긴 이유** — 목록의 모든 항목이 **모델 가중치를 바꾸지 않고 출력을 바꾸는 레버**다. MLOps의 버전
  대상이 데이터·모델이었다면, LLMOps의 버전 표면은 프롬프트·검색 설정·인덱스·평가셋까지 넓다. 재현성(같은 질문 → 같은
  답)을 확보하려면 이 전부를 한 시점의 스냅샷으로 기록해야 한다. *(위키의 관찰)* → [[MLOps]] · [[LLMOps]]
- **비용 통제는 [[지연 시간과 처리량]]의 맞교환에 토큰 비용이라는 세 번째 축을 더한다.** top-k를 늘리면 품질이 오르지만
  토큰·지연이 늘고, 캐시는 신선도(지식 최신성)와 맞바꾼다. 모델 라우팅은 서빙 쪽의 "CPU로 충분하면 GPU를 쓰지 않는다"
  ([[AI DE 강의 2-09 서빙의 CPU·GPU 가속]])와 같은 판단이다. *(위키의 연결)*
