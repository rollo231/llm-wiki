...
- **온라인 서빙에서 어려운 것은 모델이 아니라 피처 접근이다.** 이 강의는 "Feature 조회 시간이 latency의 대부분"이라고
  하고, 그 비용을 줄이는 수단으로 사전 계산·캐시·fallback을 든다. 그런데 캐시된 값·fallback 값은 **학습 때 본 값과 다른
  입력**이다 — 바로 다음 강의 [[AI DE 강의 2-06 Training-Serving Skew 예방]]이 경고하는 partial window·결측 처리 차이가 여기서
  생긴다. [[피처 스토어]]의 온라인 스토어와 [[학습-서빙 스큐]]가 만나는 지점이다. *(위키의 연결)*
