...
- **DE의 몫은 2단계에 그치지 않는다.** 표에서 "데이터 엔지니어링"은 2단계 하나지만, 그 관심사가 뒤에서 다시 나온다 —
  학습/추론 정합성은 4단계 배포([[학습-서빙 스큐]]), 데이터 버전은 3단계 실험 재현([[데이터와 모델 버전 관리]]),
  분포 변화 감지는 5단계([[데이터 드리프트]]). p17의 DE 책임 목록이 단계를 가로지르는 이유다. *(위키의 관찰)*
...
- Part 1의 Self-Healing 파이프라인 5단계(수집 → 학습 → 평가 → 배포 → 관측)는 이 6단계 가운데 2~5단계를 자동화한
  고리에 해당한다. Part 1에 없던 것은 **양 끝(1 문제 정의, 6 비즈니스 분석)**이다. *(위키의 관찰)*
