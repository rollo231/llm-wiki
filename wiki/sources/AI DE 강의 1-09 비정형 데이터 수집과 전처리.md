...
- **저장 이원화** — "무거운 몸체(데이터)는 S3에, 가벼운 영혼(정보)은 DB에." 원본 파일과 그것을 찾게 해 주는
  메타데이터를 분리하는 원리는 레이크하우스(파일 + 트랜잭션 로그)와 카탈로그(데이터 + 메타데이터)에서도 같다.
  *(위키의 연결)* → [[비정형 데이터 파이프라인]]
- 처리 단계는 정형 ETL의 Transform이 확장된 것이다 — 정제·PII 마스킹은 그대로, 여기에 OCR과 임베딩이 붙는다.
  → [[ETL과 ELT]]
- Part 1에서 **AI 고유의 부품(임베딩·벡터 DB·RAG)이 처음이자 유일하게 등장**한다. 다만 한 장씩 소개 수준이다.
