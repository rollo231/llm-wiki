...
- ⭐⭐ [[Adopting SpatialData - schema not storage]] — **포맷 채택 판단을 아키텍처 층위로 환원한 노트.**
  *"이 포맷이 내 아키텍처의 어느 층을 채우는가"* 를 물으면 답이 나온다 — 파일 포맷은 채우고,
  테이블 포맷·카탈로그·질의 엔진·오케스트레이션은 채우지 않는다. ⭐ **빈 층을 채우지 않는 것은
  인프라가 아니라 의존성이다.** 도메인 포맷 일반에 쓸 수 있는 판단 틀.
...
2026). 장 트래커(11장 / 개념 90개 / 104p). **인제스트 단위 = 장 1개 = source 페이지 1개. 전 장 완주 (11/11, 2026-08-19).**

⭐⭐ **총평·회계·다음에 읽을 것: [[Apache technology map - what it gave and what it did not]]** —
*"깊이를 팔아 판단 축을 샀다."* 새 페이지 28장 + 기존 24곳 갱신, 열린 질문 2개 완전 해소,
**출처 없는 수치 11장 전부 0건.**

강의와 역할이 다르다 — **개념당 1페이지(≈500자)라 깊이가 없고, 대신 넓이와 선택 기준을 준다.**
Tier 1/2 라벨 + `A vs B` 비교 절 7개가 실질이고, **그 7개 전부가 명시적으로 성능 순위를 거부한다.**
⭐ 착수 시점에 **개념 90개 중 42개가 이 위키에 관련 페이지조차 없었고**, 빈 칸이 Ch8(7/10)·Ch7(7/10)·
Ch11(7/9)·Ch9(4/11)·Ch2(4/7)에 몰려 있었다 — [[Wiki gap analysis - DE readiness]]가 지목한
**운영 도구** 축과 정확히 겹친다. **완주 후 실질 공백은 0이다**(남은 6개는 Ch1의 책 자체 프레임 5개 +
Commons Math 1개).

⚠️ 다만 **채운 것은 이름과 배치이고 구조는 아니다** — 11장 연속 내부 동작이 없고, 세 번은 **위키가
소스보다 자세했다**(Iceberg 매니페스트·Kafka·Parquet). *Iceberg 1차 문서* 는 여전히 1순위다.
...
| Ch11 | **특화 분석과 공통 라이브러리** | [[Apache Map - Ch11 Specialized analytics and libraries]] ⭐⭐ |
