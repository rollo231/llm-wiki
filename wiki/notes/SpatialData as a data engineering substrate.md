...
- **자매 노트** — [[Adopting SpatialData - schema not storage]]: 이 노트가 *"포맷이 무엇을 주고 안 주는가"* 라면 저쪽은
  ***"그래서 어느 부분을 채택하나"*** 다. §6의 "두 가지 청킹" 논리를 **표현 축**으로 확장하고,
  §7의 ❌ 목록에 *"이미지 Zarr 변환을 기본값으로 삼지 않는다"* 를 추가한다.
