...
AI DE(모델 학습·추론 지원, 비정형 데이터)로의 전환을 다룬다. **5개 파트 / 40개 슬라이드 덱 /
...
순서다. `raw/` 는 gitignore 대상이므로 이 배치는 로컬 캐시에만 존재한다.

| 디렉토리 | 덱 수 | 번호 | 파일명 형태 |
|---|---|---|---|
| `part1/` | 25 | `01.`~`25.` | 원본 파일명 유지 (`01. CH01-1. …` / `16. AI 모델의 적- …`) |
| `part2/` | 5 | `01.`~`05.` | `NN. Ch<N>. <챕터 제목>` — 제목은 덱에서 가져와 붙였다 |
| `part3/` | 5 | `01.`~`05.` | 〃 |
| `part4/` | 2 | `01.`~`02.` | `01. Ch1~4. …` (한 PDF에 4챕터) · `02. Ch5. …` |
| `part5/` | 3 | `01.`~`03.` | 원본 제목 유지, 중복된 원본 번호만 제거 |

**번호는 위키가 붙인 것이다.** 원본에는 파트 표기가 없었고 파일명 규칙도 파트마다 달라 순서를
복원해야 했다. 복원 근거는 아래 원본 파일명 규칙이다 — 재정리 뒤에도 근거로서 보존한다.

| 원본 파일명 형태 | 예시 | 실제 소속 | 현재 위치 |
|---|---|---|---|
| `CH0N-M.` 접두 | `CH02-7. 데이터의 시간 여행…` | Part 1 전반부(CH01~CH04) | `part1/01.`~`15.` |
| **번호만** `N.` 접두 | `4. 데이터 엔지니어의 약속 SLA…` | **Part 1 후반부** (CH04 다음) | `part1/16.`~`25.` |
| `Part2_Ch N` / `Part 3_Ch N` / `Part 4_Ch N` | `Part 3_Ch 2.pdf` | 표기 그대로 | `part2/`~`part4/` |
| `01.` / `1.` + LLM·RAG 주제 | `01. LLM과 RAG.pdf` | Part 5 | `part5/` |
...
- Part 5 소속도 슬라이드에 표기가 없다 — 사람의 배치 판단(2026-08-01). Part 5 덱 3개는 원본
  파일명이 `01.` · `01.` · `1.` 로 전부 1번이라 **덱 사이 순서도 위키의 판단이다.**
- `part1/01.` 의 원본 파일명은 `ch01-1-de-vs-ai-de-ot.pdf` 였다 — 덱의 실제 제목은
  "기존 DE vs AI DE: 데이터 적재에서 모델 지원으로의 변화".
